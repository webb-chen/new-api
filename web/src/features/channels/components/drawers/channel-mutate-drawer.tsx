...
export function ChannelMutateDrawer({
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      toast.warning(
        t(
          'Warning: Base URL should not end with /v1. Webb API will handle it automatically. This may cause request failures.'
        ),
        { duration: 5000 }
      )
    }, 500)
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    {currentType !== CHANNEL_TYPE_TASK_PLUGIN && (
                      <FormDescription>
                        {t(
                          'Custom API base URL. For official channels, Webb API has built-in addresses. Only fill this for third-party proxy sites or special endpoints. Do not add /v1 or trailing slash.'
                        )}
                      </FormDescription>
                    )}
                    {currentType === CHANNEL_TYPE_TASK_PLUGIN &&
